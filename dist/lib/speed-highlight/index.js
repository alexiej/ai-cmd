...
export async function tokenize(src, lang, token) {
    try {
        let m, part, first = {}, match, cache = [], i = 0, data = typeof lang === "string"
            ? await (langs[lang] ?? (langs[lang] = import(`./languages/${lang}.js`)))
            : lang, 
        ...
        while (i < src.length) {
            ...
            for (m = arr.length; m-- > 0;) {
                ...
                if (cache[m].match[0] &&
                    (cache[m].match.index <= first.index || first.index === null))
                    first = {
                        part: part,
            ...
            i = first.end;
            if (first.part.sub)
                await tokenize(first.match, typeof first.part.sub === "string"
                    ? first.part.sub
 ...
 * @param {ShjOptions} [opt={}] Customization options
 */
export async function highlightElementCode(elm, txt, lang = elm.className.match(/shj-lang-([\w-]+)/)?.[1], mode, opt) {
    mode ?? (mode = `${elm.tagName == "CODE" ? "in" : txt.split("\n").length < 2 ? "one" : "multi"}line`);
 ...
 * @param {ShjOptions} [opt={}] Customization options
 */
export let highlightAll = async (opt) => Promise.all(Array.from(document.querySelectorAll('[class*="shj-lang-"]')).map((elm) => highlightElement(elm, undefined, undefined, opt)));
/**
