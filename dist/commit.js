    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (needsStaging) {
                ...
                const addFilesConfirmation = yield confirm({
            ...
            const issueNumber = yield input({
                message: "Enter issue number or link (optional):",
            });
            const issueTitle = yield input({
...
- Format should be in line with the Conventional Commits standard (e.g., "<type>[scope]: <description>").`;
            ...
            console.log(`${text.dim("Using")} ${text.orange(model.source + " " + model.modelName)} ${text.dim(" to generate commit message..\n")}`);
            // Step 6: Use Model to generate the commit message
            const promptResult = (yield model.prompt(chatGptPrompt))
            ...
            const messageConfirmation = yield confirm({
                message: "Do you want to use this commit message?",
                default: true,
            });
            let finalMessage = message;
            if (!messageConfirmation) {
                finalMessage = yield input({
