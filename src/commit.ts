  ...
  try {
    ...
    const coloredFiles = modifiedFiles
      ...
      .reduce(
        ...
        { staged: [] as string[], unstaged: [] as string[] },
      );
    ...
    console.log(
      stagedFiles.length > 0 ? stagedFiles : text.gray("No staged files.\n"),
    );
    ...
    if (needsStaging) {
      ...
      if (addFilesConfirmation) {
        ...
        console.log(
          text.yellow("No files were added. Proceeding to commit generation."),
        );
      }
    }
    ...
    if (!gitDiff.trim()) {
      console.log(
        text.red(
          "No changes are staged for commit. Please stage changes using git add.",
        ),
      );
      process.exit(1);
    }
...
- Format should be in line with the Conventional Commits standard (e.g., "<type>[scope]: <description>").`;
    ...
    if (!titleConfirmation) {
      finalTitle = await input({
        ...
        validate: (value) =>
          value.length > 0 ? true : "Title cannot be empty.",
      });
    }
