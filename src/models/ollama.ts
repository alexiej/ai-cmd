...
type OllamaModelConfig = ModelConfig & { host: string };

export const OllamaModelNames: { name: string; value: string }[] = [
  {
...
export class OllamaModel implements Model {
  ...
  private static instance: OllamaModel;
  ...
  public static getInstance(): OllamaModel {
  ...
  async initialize(host: string) {
    ...
    try {
      ...
      this.status = {
        status: "running",
        message: text.green(
          ` [Ollama is live - ${models.models.length} models available] `,
        ),
      };
    } catch (error) {
      this.ollama = null;
  ...
  async init(config: ModelConfig): Promise<Model> {
    try {
