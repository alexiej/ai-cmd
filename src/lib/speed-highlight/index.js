...
export async function tokenize(src, lang, token) {
  try {
    let m,
      ...
      data =
        typeof lang === "string"
          ? await (langs[lang] ??= import(`./languages/${lang}.js`))
          : lang,
...
export async function highlightText(src, lang, multiline = true, opt = {}) {
  ...
  await tokenize(
    src,
    lang,
    (str, type) => (tmp += toSpan(sanitize(str), type)),
  );
 ...
 * @param {ShjOptions} [opt={}] Customization options
 */
export async function highlightElementCode(
  elm,
  txt,
...
export let highlightAll = async (opt) =>
  Promise.all(
    Array.from(document.querySelectorAll('[class*="shj-lang-"]')).map((elm) =>
